    ...
    const fetchPage = useCallback(async () => {
        ...
        try {
            ...
            if (severityFilter && severityFilter !== 'all') {
                if (severityFilter === 'Critical') query = query.gte('severity', 5);
                else if (severityFilter === 'High') query = query.gte('severity', 4);
                else if (severityFilter === 'Moderate') query = query.eq('severity', 3);
                else if (severityFilter === 'Minor') query = query.eq('severity', 2);
