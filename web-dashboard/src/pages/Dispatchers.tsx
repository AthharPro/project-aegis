...
import { Search, Shield, Phone, CreditCard, User } from 'lucide-react';
...
const Officers: React.FC = () => {
  ...
  const getRoleBadge = (role: string) => {
    ...
    return 'bg-slate-500/20 text-slate-400 border-slate-500/50'; // Default Officer
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-3">
             <div className="text-xs font-mono text-slate-500 bg-slate-800 px-3 py-2 rounded border border-slate-700 flex items-center gap-2">
                <Shield size={14} />
                ACTIVE UNITS: {officers.length}
             </div>
          </div>
        </div>
